...
- **Current Objective / 当前活跃功能**: M3 体验版进行中 (7/10 完成)；F-033 图片资源文件化 + F-034 视频插入已完成
- **Current State**: M1/M2/M2-ext 完成 (100%), M3 进行中 (7/10), M4 未开始 (0%), M5 完成 (100%)
- **Blockers / 阻塞项**: 无
- **Recommended Next Step / 下一步操作**:
  1. M3: F-020 日记日历视图 / F-021 全文搜索 / F-022 标签系统
  2. M4: F-030 导出 PDF/Markdown / F-031 主题切换 / F-032 自动保存增强
- **Files Modified / 最近修改文件**: `main.js`, `preload.js`, `file-store.js`, `js/editor-core.js`, `js/insert-features.js`, `js/image-resize.js`, `js/state.js`, `js/init.js`, `index.html`, `keybindings-store.js`, `feature_list.json`
...
### 2026-07-08 — 图片资源文件化 + 视频插入

**目标**: 将图片/绘画从 base64 内联改为文件存储 + 新增视频插入功能。

**完成内容**:

1. ✅ **F-033 图片/绘画资源文件化存储**
   - `main.js`: 新增 `asset:save-base64`、`asset:save-drawing`、`asset:copy-file`、`asset:get-dir` 4 个 IPC handler
   - `preload.js`: 暴露 `saveBase64Asset`、`saveDrawingAsset`、`copyAssetFile`、`getAssetDir` API
   - `file-store.js`: 新增资源文件夹管理函数（`saveBase64Asset`、`saveDrawingAsset`、`copyAssetFile`、`getAssetDir`、`deleteAssetDir`、`copyDirectorySync`）
   - `js/editor-core.js`: 新增 `extractImagesFromHTML()` 和 `resolveAssetPaths()`；改造 4 个保存调用点；改造 `saveDrawCanvasData()` 和 `initDrawCanvas()`；`selectNote()` 添加向后兼容自动迁移
   - `js/insert-features.js`: 改造 6 种图片插入路径（5 方法 + paste）和窗口截图回调，先保存到资源文件夹再插入
   - `js/image-resize.js`: 裁剪确认回调改为保存到资源文件夹
   - `index.html`: CSP 更新（`img-src` 和 `media-src` 添加 `file:`）
   - `file-store.js`: `renameNote`/`moveToTrash`/`restoreFromTrash`/`permanentlyDelete`/`emptyTrash`/`duplicateNote` 同步操作资源文件夹
2. ✅ **F-034 视频插入**
   - `main.js`: 新增 `video:open-file` IPC handler
   - `preload.js`: 暴露 `openVideoFile` API
   - `index.html`: 插入标签页添加 🎬 视频按钮
   - `js/state.js`: 添加 `ND.btnInsertVideo` DOM 引用
   - `js/insert-features.js`: 新增 `insertVideoAtCursor()` + 按钮事件绑定
   - `js/init.js`: 快捷键表添加 `insert.video` 动作
   - `keybindings-store.js`: 默认快捷键 `Ctrl+Shift+V`

**验证结果**:
- `npm test`: 216 tests passed (12 test files)
- 资源目录结构: `{userData}/notes/assets/{note-basename}/`
- 笔记文件中图片引用从 `data:image/...;base64,...` 变为 `assets/{timestamp}_{random}.png`
