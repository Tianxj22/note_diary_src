...
- **Current Objective / 当前活跃功能**: M3 里程碑待启动（下一个功能 F-020 日记日历视图）；M2 扩展功能已全部完成
- **Current State**: M1/M2 + M2-ext 完成 (100%), M3/M4 未开始 (0%)
- **Blockers / 阻塞项**: 无
- **Recommended Next Step / 下一步操作**:
  1. 阅读 `feature_list.json` 了解 F-020 日记日历视图详情
  2. 设计日历组件 UI 布局
  3. 实现日历视图的 IPC 通道和渲染
- **Files Modified / 最近修改文件**: `js/image-resize.js` (裁剪矩形缩放手柄), `index.html` (图片编辑标签页)
- **Last Updated / 最后更新**: 2026-06-26
