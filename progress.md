...
- **Current Objective / 当前活跃功能**: M3 体验版进行中 (8/11 完成)；Bug 修复完成
- **Current State**: M1/M2/M2-ext 完成 (100%), M3 进行中 (8/11 ≈ 73%), M4 未开始 (0%), M5 完成 (100%)
- **Blockers / 阻塞项**: 无
...
### 2026-07-10 — Bug 修复: 重命名笔记后图片/视频无法显示

**问题**: 重命名笔记后，图片和视频立即无法显示；Git 同步后其他设备也看不到媒体文件。

**根因分析**:

`renameNote()` 正确地将笔记文件和资源目录从 `assets/{oldBasename}/` 重命名为 `assets/{newBasename}/`，但编辑器 DOM 中图片/视频的 `file:///` URL 仍然指向旧目录（已被删除）。

恶性链条:
1. 重命名 → 资源目录物理移动 → DOM 中 file:// URL 指向已删除的旧目录 → 立即可见损坏
2. autoSave 触发 `extractImagesFromHTML()` → `fileAssetsPrefix` 为新前缀，与旧 `file:///` URL 不匹配 → URL 不被转换为相对路径 → 绝对路径原样写入文件
3. Git 同步 → 其他设备加载笔记 → `resolveAssetPaths()` 只处理 `assets/` 相对路径 → 绝对 `file:///` URL 无法解析 → 媒体永久损坏（绝对路径机器特定）

**修复内容**:

1. ✅ `js/editor-core.js` — 重命名成功后重新解析编辑器 DOM 中所有 `file:///` 资源路径:
   - 提取旧 URL 中的文件名 → 与新资源目录拼接 → 更新 img/video src
   - 同步更新裁剪图片的 `dataset.originalSrc`

2. ✅ `js/editor-core.js` `extractImagesFromHTML()` — 防御性处理:
   - 对不匹配当前笔记前缀的 `file:///` URL，提取文件名转为 `assets/xxx` 相对路径（而非原样保留绝对路径）

3. ✅ `js/editor-core.js` `resolveAssetPaths()` — 加载时防御性修复:
   - 对笔记中残留的 `file:///` URL（因上述 bug 已写入文件），提取文件名并解析到当前资源目录

4. ✅ `test/vitest.config.mjs` — 添加 `fileParallelism: false`（Node v24 + vitest 4.x 兼容）

**测试结果**: `npm test` — 242 tests passed (14 test files)

---

### 2026-07-10 — Bug 修复: HTML 实体多重编码导致笔记乱码

**问题**: 用户打开应用后发现原先编辑好的笔记变成乱码。具体表现为 HTML 标签被多重实体编码（例如 `<div>` 变为 `&amp;amp;amp;lt;div&amp;amp;amp;gt;`），每层编码叠加导致内容不可读。

**根因分析**:

`js/editor-core.js` 的 `selectNote()` 函数第 52 行使用正则 `/^\s*</.test()` 作为启发式规则判断内容是否为 HTML。该规则假设「有效 HTML 必然以 `<` 开头」，但 contenteditable 编辑器的 `innerHTML` 可以以文本节点开头（例如用户在编辑器根级直接输入中文），导致验证失败。

当正则不匹配时，代码错误地调用 `escapeHtml()` 将整个 HTML 内容当作纯文本进行实体转义，形成恶性循环：每次保存-加载叠加一层编码 → 4 次循环后即出现用户看到的 4 层编码。

**修复内容**:

1. ✅ `js/editor-core.js` — 格式检测逻辑修复:
   - 旧代码: `if (!/^\s*</.test(content))` → 对所有不以 `<` 开头的内容调用 escapeHtml
   - 新代码: `if (!decoded.metadata && !/^\s*</.test(content))` → 仅对**无元数据头**的旧格式文件执行兼容转换
   - 原理: F-041 之后所有文件都有 `<!--{JSON}-->` 元数据头，有元数据 = HTML 格式，无需启发式检测

2. ✅ `js/editor-core.js` — 新增 `decodeCorruptedEntities()` 自动修复函数:
   - 检测条件: 有元数据头 + 不以 `<` 开头 + 包含 `&amp;lt;` 等模式
   - 修复方式: 文本级逐层剥离 HTML 实体编码（最多 10 层安全上限）
   - 修复后强制触发保存以持久化恢复结果

3. ✅ `js/editor-core.js` — `lastSavedContent` 基准计算改用 `ND.currentContent`（可能已被修复）而非 `decoded.text`（原始文件内容）

4. ✅ `test/unit/utils.test.mjs` — 新增 11 条测试用例 (U-129 ~ U-139):
   - 正常 HTML 幂等性、1/2/4 层编码解码、中文+编码混合、纯文本、空字符串、属性引号实体、已修复内容二次幂等

5. ✅ `test/vitest.config.mjs` — `pool: 'forks'` → `pool: 'threads'`（Node v24 + vitest 4.x 兼容修复）

**测试结果**: `npm test` — 242 tests passed (14 test files, +11 new)

---

### 2026-07-08 — 应用自动更新 (F-051) + 发布流程文档

**目标**: 添加基于 electron-updater + GitHub Releases 的应用自动更新功能；编写发布流程文档。

**完成内容**:

1. ✅ `package.json` — 版本号 `1.0.0` → `1.11.0`，新增 `electron-updater` 依赖，`build.publish` 指向 GitHub Releases
2. ✅ `update-checker.js` — 新建更新检查模块，封装 `autoUpdater` 事件（checking/available/progress/downloaded/error）→ `update:status` IPC 推送
3. ✅ `main.js` — 注册 `update:check` / `update:get-version` / `update:install` IPC handler；启动时调用 `updateChecker.init()`
4. ✅ `preload.js` — 暴露 `checkForUpdate` / `getAppVersion` / `installUpdate` / `onUpdateStatus` API
5. ✅ `index.html` — 设置弹窗新增「更新」区域：版本号 + 检查按钮 + 进度条 + 安装/稍后按钮
6. ✅ `css/settings.css` — 更新进度条 + 状态文字 + 操作区域样式
7. ✅ `js/settings-ui.js` — 更新 UI 逻辑：`populateVersion()` / `handleUpdateStatus()` 状态机 / `resetUpdateUI()`
8. ✅ `docs/git_workflow.md` — 新增 §8「发布新版本（GitHub Release）」完整操作流程
9. ✅ `feature_list.json` — 新增 F-051，status=done，completedAt=2026-07-08

**关键架构决策**:
- `autoUpdater.autoDownload = true`：检测到更新后自动下载，无需用户二次确认
- 更新状态通过 `webContents.send('update:status', data)` 推送，渲染进程通过 `onUpdateStatus(callback)` 监听
- 仅手动触发检查（点击设置弹窗内按钮），无自动后台检查
- 发布流程为手动操作（bump version → build → upload to GitHub Releases），记录在 `docs/git_workflow.md`

**验证结果**: `npm test` — 221 tests passed (13 test files)

---

### 2026-07-08 — 图片/视频资源文件化 + 视频插入/编辑 + 媒体操作增强

**目标**: 图片/绘画 base64 → 文件存储、视频插入与编辑、媒体操作增强。

**完成内容**:

1. ✅ **F-033 图片/绘画资源文件化存储** — 笔记文件仅存储相对路径 `assets/xxx.png`
2. ✅ **F-034 视频插入** — 工具栏按钮 + Ctrl+Shift+V，本地视频文件选择，`<video>` 嵌入
3. ✅ **视频缩放 + 裁剪** — 复用图片编辑系统：8 手柄缩放、宽高输入、宽高比锁定、CSS 容器裁剪（非破坏性）、恢复原尺寸
4. ✅ **媒体操作增强**:
   - Delete/Backspace 删除选中的图片/视频
   - 插入标签页所有内容（清单/时间戳/列表/图片/视频/代码块）插入后自动添加空行
   - Ctrl+C / 右键菜单复制图片到系统剪贴板
   - 视频复制粘贴跨笔记支持（`__NDVIDEO__` 文本前缀 + `copyAssetFile`）
   - 插入后 `file:///` 立即显示（无需关闭重开）
5. ✅ **测试** — 新增 `clipboard-media.test.mjs`（5 用例），全部 221 测试通过

**关键架构决策**:
- 资源目录: `{userData}/notes/assets/{note-basename}/`，从笔记文件名推导
- 插入时立即保存为文件（非延迟），`resolveDisplaySrc()` 转为 `file:///` URL 即时显示
- 保存时 `extractImagesFromHTML()` 将 `file:///` 还原为相对路径，保证笔记文件可移植
- 视频裁剪用 CSS `overflow:hidden` wrapper，`data-original-html` 保留原始 HTML 用于恢复
...
### 2026-06-29 — Electron-builder 跨平台打包与快捷方式生成 (F-050)

**目标**: 引入 electron-builder 实现跨平台打包，自动生成桌面快捷方式。

**完成内容**:

1. ✅ `npm install --save-dev electron-builder` — 安装打包工具
2. ✅ `scripts/resize-icon.js` — 图标缩放脚本（从 `icons.png` 生成各平台所需尺寸）
3. ✅ `build/icon.png` + `build/icons/` — 多尺寸图标就位（16/32/48/256/512）
4. ✅ `package.json` — 新增 `build`/`build:win`/`build:mac`/`build:linux`/`icon:prepare` 脚本 + electron-builder 完整配置
5. ✅ `main.js` — 加固：
   - `app.setName('Note Diary')` + Windows `app.setAppUserModelId()`
   - `app.requestSingleInstanceLock()` 单实例锁
   - BrowserWindow `icon` 属性
   - 托盘图标改用 `build/icon.png` 文件（替代硬编码 base64 蓝色方块）
6. ✅ `README.md` — 项目说明文档（功能、技术栈、开发/构建/测试指令、项目结构）
7. ✅ `init.sh` — 静态检查增加 `build/icon.png` 存在性验证
8. ✅ `feature_list.json` — 新增 F-050, status=done

**打包产物（Windows）**:
- NSIS 安装器：`dist/Note-Diary-Setup-x.x.x.exe`（桌面 + 开始菜单快捷方式）
- 便携版：`dist/Note-Diary-x.x.x-portable.exe`

**技术决策**:
- 图标方案：用户提供 `icons.png` → 复制到 `build/icon.png`，electron-builder 自动转换 ICO/ICNS
- NSIS 标准向导模式（非 oneClick），允许用户选择安装目录
- 跳过代码签名（需付费证书，后续补充）
- CSP 无需修改（`'self'` 在 `file://` + asar 下正常工作）

**下一步**: 继续 M3 剩余功能（F-020 日历 / F-021 搜索 / F-022 标签）或 M4
...
| 2026-07-08 | `npm test` | 221/221 全部通过 (13 test files) |
| 2026-07-10 | `npm test` | 242/242 全部通过 (14 test files, +11 实体解码测试) |
